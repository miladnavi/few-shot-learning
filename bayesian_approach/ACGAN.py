...
class ACGAN(object):
    def __init__(self, args):
        ...
        if self.gpu_mode:
            self.sample_z_, self.sample_y_ = Variable(self.sample_z_.to(device), volatile=True), Variable(self.sample_y_.to(device), volatile=True)
        else:
    ...
    def train(self):
        ...
        if self.gpu_mode:
            self.y_real_, self.y_fake_ = Variable(torch.ones(self.batch_size, 1).to(device)), Variable(torch.zeros(self.batch_size, 1).to(device))
        else:
        ...
        for epoch in range(self.epoch):
            ...
            for iter in range(len(self.data_X) // self.batch_size):
                ...
                if self.gpu_mode:
                    x_, z_, y_vec_ = Variable(x_.to(device)), Variable(z_.to(device)), Variable(y_vec_.to(device))
                else:
            ...
            for iter in range(nb_test // self.batch_size):
                ...
                if self.gpu_mode:
                    x_, y_vec_ = Variable(x_.to(device)), Variable(y_vec_.to(device))
                else:
    ...
    def visualize_results(self, epoch, fix=True):
        ...
        if fix:
            ...
            if self.gpu_mode:
                sample_z_, sample_y_ = Variable(torch.rand((self.batch_size, self.z_dim)).to(device), volatile=True), \
                                       Variable(sample_y_.to(device), volatile=True)
            else:
